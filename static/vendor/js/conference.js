...
var conference = function(config) {
    ...
    function openSubSocket(_config) {
        ...
        function onRemoteStreamStartsFlowing() {
            ...
            // console.log(HTMLMediaElement.HAVE_CURRENT_DATA);
        ...
        async function updateStream(screenVideoTrack,stream){
            console.log(peers);
            
            for(pi in peers){
                var senders = peers[pi].peer.getSenders();
                //console.log(senders);
                
                for(x in senders){
                    if(senders[x] && senders[x].track && senders[x].track.kind=="video"){
                        //console.log(senders[x]);
                        senders[x].replaceTrack(screenVideoTrack);
                    }
                    else if(!senders[x].track){
                        delete senders[x];
                    }
                }
            }

            // peer.peer.removeStream(config.attachStream);
    ...
    function updateStream(screenVideoTrack,stream){
        subSocket.updateStream(screenVideoTrack,stream);
    }
